    ...
    * If Bool, the input value is used
    ...
    def __init__(
        ...
        if self.use_trained_model:
            self.model = torch.load(model_file_name, weights_only=False)
    ...
    def get_optimal_control(
        ...
        if self.use_trained_model and drive_functions.use_memory_for_training:
            if len(time_stamp) > 0:
                if drive_functions.use_tcn_for_training:
                    # TCN: advance the context with the current real state so that the
                    # horizon rollout will see up-to-date history.  Then snapshot it so
                    # each iLQR/MPPI iteration can restore a consistent starting context.
                    self.transform_model.pred(self.X_current)
                    self.transform_model.save_base_context()
                else:
        ...
        if self.mode == "mppi" or self.mode == "mppi_ilqr":  # Run mppi
            ...
            for k in range(drive_functions.max_iter_mppi):
                ...
                if (
                    ...
                    drive_functions.use_memory_for_training
                    and self.use_trained_model
                    and drive_functions.use_tcn_for_training
                ):
                    self.transform_model.restore_base_context()
